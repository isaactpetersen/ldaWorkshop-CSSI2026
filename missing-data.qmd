...
#predictorMatrix[Y, "ses"] <- 2 # random effect predictor
#predictorMatrix[Y, "sleep"] <- 1 # random effect predictor
#predictorMatrix[Y, "depression"] <- 1 # random effect predictor
#predictorMatrix[Y, "anxiety"] <- 1 # random effect predictor
...
stripplot(mi_mice, depression ~ .imp)
```
...
```{r}
mi_mice_long <- mice::complete(
  mi_mice,
  action = "long",
  include = TRUE)

mi_mice_long <- mi_mice_long |> # for mixed-effects modeling
  group_by(ID, .imp) |>
  mutate(
...
mi_mice_wide <- mi_mice_long |> # for structural equation modeling: widen by time
